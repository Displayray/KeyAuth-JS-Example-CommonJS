/* 
    Based on: https://github.com/mazk5145/KeyAuth-JS-Example
*/
...
async function loggedin() {
  ...
  for (var i = 0; i < KeyAuth.user_data.subscriptions.length; i++) {
    console.log(
      ` [${i}] Subscription name: ${
        KeyAuth.user_data.subscriptions[i].subscription
      } | Expires at: ${moment
        .unix(KeyAuth.user_data.subscriptions[i].expiry)
        .format("DD-MM-YYYY - HH:mm:ss")} | Time left in seconds ${
        KeyAuth.user_data.subscriptions[i].timeleft
      }`
    );
  }
